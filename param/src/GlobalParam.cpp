...
{   
    cv::FileStorage fs;
    bool has_camera_tilt_in_camera_cfg = false;
    bool has_camera_roll_in_camera_cfg = false;
    this ->color = color;
    ...
    if (!fs["camera_tilt_deg"].empty())
    {
        has_camera_tilt_in_camera_cfg = true;
        fs["camera_tilt_deg"] >> camera_tilt_deg;
    }
    if (!fs["camera_roll_deg"].empty())
    {
        has_camera_roll_in_camera_cfg = true;
        fs["camera_roll_deg"] >> camera_roll_deg;
    }
    fs.release();
    ...
    if (!fs["yaw_percent"].empty())
        fs["yaw_percent"] >> yaw_percent;
    if (!fs["outpost_yaw_percent"].empty())
        fs["outpost_yaw_percent"] >> outpost_yaw_percent;
    // 卡尔曼滤波相关参数
    fs["s2qxyz"] >> s2qxyz;              // 位置转移噪声
    fs["s2qvx"] >> s2qvx;                // vx随机游走噪声
    fs["s2qvy"] >> s2qvy;                // vy随机游走噪声
    fs["s2qyaw"] >> s2qyaw;              // 角度转移噪声
    fs["s2q_vyaw"] >> s2q_vyaw;          // vyaw随机游走噪声
    fs["s2qr"] >> s2qr;                  // 半径转移噪声
    fs["r_xy_factor"] >> r_xy_factor;     
    fs["r_z"] >> r_z;     
    fs["r_yaw"] >> r_yaw;
    if (!fs["r_yaw_static"].empty())
        fs["r_yaw_static"] >> r_yaw_static;
    if (!fs["r_yaw_static_vyaw_thresh"].empty())
        fs["r_yaw_static_vyaw_thresh"] >> r_yaw_static_vyaw_thresh;
    fs["s2p0xyr"] >> s2p0xyr;
    fs["s2p0yaw"] >> s2p0yaw;
    fs["s2p0vyaw"] >> s2p0vyaw;
    fs["r_initial"] >> r_initial;
    fs["small_armor_a"] >> small_armor_a; // 小装甲板的半长
    fs["small_armor_b"] >> small_armor_b; // 小装甲板的半宽
    fs["big_armor_a"] >> big_armor_a;     // 大装甲板的半长
    fs["big_armor_b"] >> big_armor_b;     // 大装甲板的半宽
    fs["resize"] >> resize;
    // 兼容旧参数名：camera_pitch_comp（已迁移到CameraConfig.yaml中的camera_tilt_deg）
    if (!has_camera_tilt_in_camera_cfg && !fs["camera_pitch_comp"].empty())
        fs["camera_pitch_comp"] >> camera_tilt_deg;
    // 兼容旧参数名：camera_roll_comp（已迁移到CameraConfig.yaml中的camera_roll_deg）
    if (!has_camera_roll_in_camera_cfg && !fs["camera_roll_comp"].empty())
        fs["camera_roll_comp"] >> camera_roll_deg;
    // fs["r_range_factor"] >> r_range_factor; // 距离噪声与距离的比例因子
    // fs["r_range_base"] >> r_range_base;    // 基础距离噪声
    // fs["r_angle_base"] >> r_angle_base;      // 基础角度噪声
    // fs["r_angle_scale"] >> r_angle_scale;
    fs["r_yaw_obs"] >> r_yaw_obs;
    fs["r_pitch_obs"] >> r_pitch_obs;
    fs["r_dist"] >> r_dist;
    fs["r_dist_coeff"] >> r_dist_coeff;
    fs["r_dist_base"] >> r_dist_base;
    fs["outpost_r"] >> outpost_r;
    fs["outpost_armor_pitch"] >> outpost_armor_pitch;
    fs["outpost_z_offset_0"] >> outpost_z_offset_0;
    fs["outpost_z_offset_1"] >> outpost_z_offset_1;
    fs["outpost_z_offset_2"] >> outpost_z_offset_2;
    fs["outpost_s2qxy"] >> outpost_s2qxy;
    fs["outpost_s2qz"] >> outpost_s2qz;
    fs["outpost_s2qyaw"] >> outpost_s2qyaw;
    fs["outpost_s2qvyaw"] >> outpost_s2qvyaw;
    fs["outpost_s2p0xy"] >> outpost_s2p0xy;
    fs["outpost_s2p0z"] >> outpost_s2p0z;
    fs["outpost_s2p0r"] >> outpost_s2p0r;
    fs["outpost_s2p0yaw"] >> outpost_s2p0yaw;
    fs["outpost_s2p0vyaw"] >> outpost_s2p0vyaw;
    fs["outpost_r_yaw_obs"] >> outpost_r_yaw_obs;
    fs["outpost_r_pitch_obs"] >> outpost_r_pitch_obs;
    fs["outpost_r_dist_coeff"] >> outpost_r_dist_coeff;
    fs["outpost_r_dist_base"] >> outpost_r_dist_base;
    fs["outpost_r_pose_yaw"] >> outpost_r_pose_yaw;
    
    // SoftResetNet removed: related parameters are ignored
    fs["side_r_update_noise"] >> side_r_update_noise;
    fs["side_match_threshold_mm"] >> side_match_threshold_mm;
    if (!fs["side_match_z_threshold_mm"].empty())
        fs["side_match_z_threshold_mm"] >> side_match_z_threshold_mm;
    
    // 双EKF竞争重建参数
    fs["enable_dual_ekf"] >> enable_dual_ekf;
    fs["dual_ekf_observe_frames"] >> dual_ekf_observe_frames;
    fs["dual_ekf_decay"] >> dual_ekf_decay;
    fs["dual_ekf_velocity_memory_frames"] >> dual_ekf_velocity_memory_frames;
    fs.release();
    ...
    // 孤儿灯条参数
    fs["orphan_min_ratio"] >> orphan_min_ratio;
    fs["orphan_max_ratio"] >> orphan_max_ratio;
    fs["orphan_max_angle"] >> orphan_max_angle;
    fs["orphan_min_length"] >> orphan_min_length;
    fs["orphan_max_area"] >> orphan_max_area;
    fs.release();
    ...
    if (!fs.open("../config/WindmillConfig.yaml", cv::FileStorage::READ)) {
      ...
      fs["windmill_blue_exp_time"] >> windmill_blue_exp_time;
      fs["windmill_red_exp_time"] >> windmill_red_exp_time;
...
void GlobalParam::saveGlobalParam()
{
    ...
    // SoftResetNet removed: related parameters are not saved
